...
extern int isprint(unsigned char ch); /* from shell.c */
...
static rt_bool_t rtgui_edit_onfocus(pvoid wdt, rtgui_event_t* event);
...
static rt_bool_t rtgui_edit_onfocus(pvoid wdt, rtgui_event_t* event)
{
	...
	edit->caret_timer = rtgui_timer_create("edit", 100, RT_TIMER_FLAG_PERIODIC,
...
rt_uint32_t rtgui_edit_get_mem_consume(struct rtgui_edit *edit)
{
	...
	mem_size += edit->col_per_page + 1;
	/* mem_size += (edit->font_width*2+1) * (edit->font_height+1) * sizeof(rtgui_color_t) + 
				sizeof(struct rtgui_dc_buffer); */
	if(edit->head != RT_NULL)
	{
		line = edit->head;
		while(line)
