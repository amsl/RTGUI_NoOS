...
static void rtgui_hz_bitmap_font_draw_text(struct rtgui_font* font, rtgui_dc_t *dc, const char* text, rt_uint32_t len, rtgui_rect_t* rect);
...
static void _rtgui_hz_bitmap_font_draw_text(struct rtgui_font_bitmap* bmp_font, rtgui_dc_t *dc, const char* text, rt_ubase_t len, rtgui_rect_t* rect)
{
	...
	while(len > 0 && rect->x1 < rect->x2)
	{
		...
		/* calculate GBK section and index */
		sect = *str - 0x81;
		index = *(str+1) - 0x40;

		/* get font pixel data */
		font_ptr = bmp_font->bmp + (sect*192 + index)*(bmp_font->width + bmp_font->height);
...
static void rtgui_hz_bitmap_font_draw_text(struct rtgui_font* font, rtgui_dc_t *dc, const char* text, rt_ubase_t length, rtgui_rect_t* rect)
{
	rt_uint32_t len;
	...
	while(length > 0)
	{
		...
		len = 0;
		while(((rt_uint8_t)*(text + len)) >= 0x81) len += 2;
