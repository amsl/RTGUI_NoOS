...
static void rtgui_bitmap_font_draw_text(struct rtgui_font* font, rtgui_dc_t *dc, const char* text, rt_uint32_t len, rtgui_rect_t* rect);
...
static void rtgui_bitmap_font_draw_text(struct rtgui_font* font, rtgui_dc_t *dc, const char* text, rt_uint32_t len, rtgui_rect_t* rect)
{
	...
	hz_font = rtgui_font_refer("GBK", font->height);
	while((rect->x1 < rect->x2) && len)
	{
		length = 0;
		while((rt_uint8_t)*(text + length) >= 0x81) length += 2;  /* it's not a ascii character */
		if(length > 0)
		{
			if(hz_font != RT_NULL)
