...
void rtgui_dc_draw_border(rtgui_dc_t* dc, rtgui_rect_t* rect, int flag)
{
	...
	switch(flag)
	{
		case RTGUI_BORDER_RAISE:
			rtgui_dc_draw_shaded_rect(dc, &r, theme.blankspace, DimGray);
			rtgui_rect_inflate(&r, -1);
			rtgui_dc_draw_shaded_rect(dc, &r, theme.background, Gray);
			break;

		case RTGUI_BORDER_SUNKEN:
			rtgui_dc_draw_shaded_rect(dc, &r, Gray, theme.blankspace);
			rtgui_rect_inflate(&r, -1);
			rtgui_dc_draw_shaded_rect(dc, &r, DimGray, theme.background);
			break;

		case RTGUI_BORDER_BOX:
			rtgui_dc_draw_shaded_rect(dc, &r, Gray, theme.blankspace);
			rtgui_rect_inflate(&r, -1);
			rtgui_dc_draw_shaded_rect(dc, &r, theme.blankspace, Gray);
			break;

		case RTGUI_BORDER_STATIC:
			//rtgui_dc_draw_shaded_rect(dc, &r, Gray, theme.blankspace);
			RTGUI_DC_FC(dc) = theme.blankspace;
			rtgui_dc_draw_hline(dc, r.x1, r.x2-1, r.y1+1);
			rtgui_dc_draw_hline(dc, r.x1, r.x2, r.y2);
			rtgui_dc_draw_vline(dc, r.x1+1, r.y1, r.y2);
			rtgui_dc_draw_vline(dc, r.x2-1, r.y1, r.y2);
			RTGUI_DC_FC(dc) = Gray;
			rtgui_dc_draw_hline(dc, r.x1, r.x2-1, r.y1);
			rtgui_dc_draw_hline(dc, r.x1, r.x2-1, r.y2-1);
			rtgui_dc_draw_vline(dc, r.x1, r.y1, r.y2);
			rtgui_dc_draw_vline(dc, r.x2-2, r.y1, r.y2);
			break;

		case RTGUI_BORDER_EXTRA:
			rtgui_dc_draw_shaded_rect(dc, &r, Silver, Black);
			rtgui_rect_inflate(&r, -1);
			rtgui_dc_draw_shaded_rect(dc, &r, theme.blankspace, DimGray);
			break;
		...
		case RTGUI_BORDER_DOWN:
			rtgui_dc_draw_shaded_rect(dc, &r, DimGray, theme.blankspace);
			break;
		default:
			break;
	}
...
void rtgui_dc_draw_regular_polygon(rtgui_dc_t* dc, int x, int y, int r, int count, rt_uint16_t angle)
{
	int i, temp_val;
	double temp;
	...
	*/
	...
	do
	{
		...
		if(oct == startoct)
		{
			...
			case 0:
			case 3:
				temp = sin(start * M_PI / 180.0);
				break;
			case 1:
			case 6:
				temp = cos(start * M_PI / 180.0);
				break;
			case 2:
			case 5:
				temp = -cos(start * M_PI / 180.0);
		...
		if(oct == endoct)
		{
			...
			case 0:
			case 3:
				temp = sin(end * M_PI / 180.0);
				break;
			case 1:
			case 6:
				temp = cos(end * M_PI / 180.0);
				break;
			case 2:
			case 5:
				temp = -cos(end * M_PI / 180.0);
...
void rtgui_dc_draw_ellipse_bre(rtgui_dc_t *dc, float xc, float yc, float fla, float flb, double angle)
{
	...
	while(dx<dy)
	{
		...
		if(i==5)
		{
			rtgui_dc_draw_line(dc, flX[0], flY[0], xc+x*cosA-y*sinA, yc+y*cosA+x*sinA);
			rtgui_dc_draw_line(dc, flX[1], flY[1], xc+x*cosA+y*sinA, yc-y*cosA+x*sinA);
			rtgui_dc_draw_line(dc, flX[2], flY[2], xc-x*cosA-y*sinA, yc+y*cosA-x*sinA);
			rtgui_dc_draw_line(dc, flX[3], flY[3], xc-x*cosA+y*sinA, yc-y*cosA-x*sinA);
			flX[0] = xc+x*cosA-y*sinA;
			flX[1] = xc+x*cosA+y*sinA;
			flX[2] = xc-x*cosA-y*sinA;
			flX[3] = xc-x*cosA+y*sinA;
