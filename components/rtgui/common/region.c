...
#define allocData(n) rt_malloc(PIXREGION_SZOF(n))
#define freeData(reg) if ((reg)->data && (reg)->data->size) rt_free((reg)->data)
        ...
        int *pOverlap)
{
	...
	*pOverlap = RTGUI_REGION_STATUS_FAILURE;
	...
	ri = (RegionInfo *) rt_malloc(4 * sizeof(RegionInfo));
